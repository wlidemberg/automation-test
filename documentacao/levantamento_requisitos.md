...
*   **DA-009 (Fluxo de Onboarding Técnico com Agente de IA & n8n)**: Introdução da etapa de Briefing Técnico (`BriefingPage.tsx`) com rota pública `/briefing/:projectId` para coleta estruturada de requisitos visuais e métricas operacionais do cliente (faturamento, equipe, público, dores), enviada ao n8n para precificação e proposta dinâmica gerada pela IA, visível em `/proposta/:briefingId`.
*   **DA-010 (Funil e Acompanhamento de Onboarding no Painel Admin)**: O painel de gestão administrativa (`AdminOverview.tsx`) exibe um rastreador de progresso reativo e opaco de 4 etapas: **Briefing enviado**, **Proposta gerada**, **Entrada Paga** e **Senha cadastrada (Ativado)**. Os ícones mudam de opacidade e adquirem cor verde neon sólida reativa conforme cada progresso do onboarding do lead no funil se consolida no banco de dados.
*   **DA-011 (Mecanismos de Prevenção de Duplo Clique)**: Todas as ações críticas de gravação no banco de dados (envio de proposta/briefing e efetivação de faturas simuladas) contêm guards e desativações em nível de estado (`isSubmitting` / `isPaying`) para bloquear cliques simultâneos acidentais e evitar duplicidade de registros.
